...
function ZipSummary({ results }: { results: UploadFileResult[] }) {
  ...
  return (
    <Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.5, mt: 0.5 }}>
      ...
      {imported > 0 && <Chip label={`${imported} imported`} color="success" size="small" variant="outlined" />}
      {duplicate > 0 && <Chip label={`${duplicate} duplicate`} color="default" size="small" variant="outlined" />}
      {skipped > 0 && <Chip label={`${skipped} skipped`} color="warning" size="small" variant="outlined" />}
      {errors > 0 && <Chip label={`${errors} error${errors !== 1 ? "s" : ""}`} color="error" size="small" variant="outlined" />}
    </Box>
  );
}
...
export default function UploadDialog({ open, onClose }: UploadDialogProps) {
  ...
  function handleClose() {
    if (uploading) return;
    const didImport = importedCount > 0;
    setQueue([]);
    onClose(didImport);
  }
  ...
  function addFiles(files: FileList | File[]) {
    const arr = Array.from(files).filter(
      (f) => f.name.toLowerCase().endsWith(".fit") || f.name.toLowerCase().endsWith(".zip")
    );
    if (arr.length === 0) return;
    setQueue((prev) => [...prev, ...arr.map((file) => ({ file, status: "pending" as FileStatus }))]);
  }
  ...
  const uploadAll = useCallback(async () => {
    ...
    setUploading(true);
    setQueue((prev) => prev.map((e) => (e.status === "pending" ? { ...e, status: "uploading" } : e)));
    ...
    try {
      ...
      if (!res.ok && !data.fileResults) {
        setQueue((prev) =>
          prev.map((e) =>
            e.status === "uploading" ? { ...e, status: "done", error: data.error ?? "Upload failed" } : e
          )
        );
        return;
      }
      ...
      setQueue((prev) =>
        prev.map((entry) => {
          ...
          if (!results) return { ...entry, status: "done", error: "No result received" };
          if (results.length === 0) return { ...entry, status: "done", error: "No .fit files found in zip" };
          return { ...entry, status: "done", results };
        })
      );
    } catch {
      setQueue((prev) =>
        prev.map((e) => (e.status === "uploading" ? { ...e, status: "done", error: "Network error" } : e))
      );
    } finally {
      setUploading(false);
    }
          ...
          <Paper variant="outlined">
            <Box sx={{ px: 2, py: 1.5, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              ...
              <Box sx={{ display: "flex", gap: 1 }}>
                {pendingCount > 0 && (
                  <Button variant="contained" size="small" onClick={uploadAll} disabled={uploading}>
