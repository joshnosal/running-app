...
export default function D3EfficiencyBandChart({ units }: { units: Units }) {
  ...
  useEffect(() => {
    ...
    fetch(url)
      ...
      .then((res) => {
        ...
        // Sort chronologically: by startTime ASC, then lapNumber ASC within same activity
  ...
  useEffect(() => {
    ...
    const mean = d3.mean(efficiencies)!;
    const sd = d3.deviation(efficiencies) ?? 0;
  ...
  }, [chartData, containerW, units, dataType]);
